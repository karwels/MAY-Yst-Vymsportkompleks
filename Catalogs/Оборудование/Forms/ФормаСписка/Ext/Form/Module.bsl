    ...
    Для Каждого Строка Из Строки Цикл
        ...
        Попытка
            ...
            Запрос.Текст = 
                ...
                |ГДЕ 
                |    Оборудование.Код = &Код";
            Запрос.УстановитьПараметр("Код", Код);
            Результат = Запрос.Выполнить();
            Выборка = Результат.Выбрать();
            
            Если Выборка.Следующий() Тогда
                СсылкаОборудования = Выборка.Ссылка;
                ОбъектОборудования = СсылкаОборудования.ПолучитьОбъект();
                Сообщить("Обновлен существующий элемент: " + Код);
            Иначе
                ОбъектОборудования = Справочники.Оборудование.СоздатьЭлемент();
                ОбъектОборудования.Код = Код;
                Сообщить("Создан новый элемент: " + Код);
            КонецЕсли;
            
            // Заполнение реквизитов
            ОбъектОборудования.Наименование = Наименование;
...
Процедура Выгрузить(Команда)
    ...
    ИмяФайлаПоУмолчанию = "Выгрузка_оборудования.txt";
