    ...
    Для Каждого Строка Из Строки Цикл
        ...
        Попытка
            ...
            Если Выборка.Следующий() Тогда
                СсылкаМенеджер = Выборка.Ссылка;
                ОбъектМенеджер = СсылкаМенеджер.ПолучитьОбъект();
                Сообщить("Обновлен существующий элемент: " + Код);
            Иначе
                ОбъектМенеджер = Справочники.Менеджеры.СоздатьЭлемент();
                ОбъектМенеджер.Код = Код;
                Сообщить("Создан новый элемент: " + Код);
            КонецЕсли;
            
            // Заполнение реквизитов
            ОбъектМенеджер.Наименование = Наименование;
            ОбъектМенеджер.ДатаРождения = ДатаРождения;
            ОбъектМенеджер.НомерТелефона = НомерТелефона;
            ОбъектМенеджер.Email = Email;
            ОбъектМенеджер.Ставка = Ставка;
            
            // Сохранение изменений
            ОбъектМенеджер.Записать();
...
Процедура Выгрузить(Команда)
    ...
    ИмяФайлаПоУмолчанию = "Выгрузка_менеджеров.txt";
...
Функция СформироватьТекстДляВыгрузкиНаСервере()
    // Получаем данные из справочника Менеджеры
    Запрос = Новый Запрос;
    Запрос.Текст = 
        "ВЫБРАТЬ
        |    Менеджеры.Код КАК Код,
        |    Менеджеры.Наименование КАК Наименование,
        |    Менеджеры.ДатаРождения КАК ДатаРождения,
        |    Менеджеры.НомерТелефона КАК НомерТелефона,
        |    Менеджеры.Email КАК Email,
        |    Менеджеры.Ставка КАК Ставка
        |ИЗ
        |    Справочник.Менеджеры КАК Менеджеры";
    ...
    // Формируем заголовки
    Заголовки = "Код;Наименование;ДатаРождения;НомерТелефона;Email;Ставка";
    ...
    Пока Выборка.Следующий() Цикл
        Результат = Результат + 
            ...
            Формат(Выборка.Ставка, "ЧДЦ=2") + Символы.ПС;
    КонецЦикла;
...
Процедура СохранитьВXLS(Путь)
    ...
    Для i = 1 По РезультатЗапроса.Количество() Цикл
        ...
        Лист.Cells(i + 1, 6).Value = Выборка.Ставка;
    КонецЦикла;
...
Процедура СохранитьВDOCX(Путь)
    ...
    // Заголовки таблицы
    Документ.Content.Text = "Код;Наименование;ДатаРождения;НомерТелефона;Email;Ставка" + Символы.ПС;
    ...
    Таблица.Cell(1, 6).Range.Text = "Ставка";
    ...
    Для i = 1 По РезультатЗапроса.Количество() Цикл
        ...
        Таблица.Cell(i + 1, 6).Range.Text = Формат(Выборка.Ставка, "ЧДЦ=2");
    КонецЦикла;
