    ...
    Для Каждого Строка Из Строки Цикл
        ...
        Попытка
            ...
            Если Выборка.Следующий() Тогда
                СсылкаЗал = Выборка.Ссылка;
                ОбъектЗал = СсылкаЗал.ПолучитьОбъект();
                Сообщить("Обновлен существующий элемент: " + Код);
            Иначе
                ОбъектЗал = Справочники.Зал.СоздатьЭлемент();
                ОбъектЗал.Код = Код;
                Сообщить("Создан новый элемент: " + Код);
            КонецЕсли;
            
            ОбъектЗал.Наименование = Наименование;
...
Процедура Выгрузить(Команда)
    ...
    ИмяФайлаПоУмолчанию = "Выгрузка_залов.txt";
...
Функция СформироватьТекстДляВыгрузкиНаСервере()
    ...
    Запрос.Текст = 
        "ВЫБРАТЬ
        |    Зал.Код КАК Код,
        |    Зал.Наименование КАК Наименование
        |ИЗ
        |    Справочник.Зал КАК Зал";
