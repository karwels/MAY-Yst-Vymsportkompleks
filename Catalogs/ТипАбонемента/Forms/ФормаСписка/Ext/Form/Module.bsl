...
Процедура ОбработатьДанныеНаСервере(Данные)
    ...
    Для Каждого Строка Из Строки Цикл
        ...
        Попытка
            ...
            СрокДействия     = Число(СокрЛП(Колонки[2]));
            ...
            Запрос.Текст = 
                ...
                |    Ссылка 
                |ИЗ 
                |    Справочник.ТипАбонемента КАК ТипАбонемента 
                |ГДЕ 
                |    ТипАбонемента.Код = &Код";
            Запрос.УстановитьПараметр("Код", Код);
            Результат = Запрос.Выполнить();
            Выборка = Результат.Выбрать();
            
            Если Выборка.Следующий() Тогда
                СсылкаТипАбонемента = Выборка.Ссылка;
                ОбъектТипАбонемента = СсылкаТипАбонемента.ПолучитьОбъект();
                Сообщить("Обновлен существующий элемент: " + Код);
            Иначе
                ОбъектТипАбонемента = Справочники.ТипАбонемента.СоздатьЭлемент();
                ОбъектТипАбонемента.Код = Код;
                Сообщить("Создан новый элемент: " + Код);
            КонецЕсли;
            
            // Заполнение реквизитов
            ОбъектТипАбонемента.Наименование = Наименование;
            ОбъектТипАбонемента.СрокДействия = СрокДействия;
            
            // Сохранение изменений
            ОбъектТипАбонемента.Записать();
...
Процедура Выгрузить(Команда)
    ...
    ИмяФайлаПоУмолчанию = "Выгрузка_типов_абонементов.txt";
...
Функция СформироватьТекстДляВыгрузкиНаСервере()
    ...
    Запрос.Текст = 
        "ВЫБРАТЬ
        |    ТипАбонемента.Код КАК Код,
        |    ТипАбонемента.Наименование КАК Наименование,
        |    ТипАбонемента.СрокДействия КАК СрокДействия
        |ИЗ
        |    Справочник.ТипАбонемента КАК ТипАбонемента";
    ...
    Заголовки = "Код;Наименование;СтрокДействия";
    ...
    Пока Выборка.Следующий() Цикл
        Результат = Результат + 
            ...
            Строка(Выборка.СрокДействия) + Символы.ПС;
    КонецЦикла;
...
Процедура СохранитьВXLS(Путь)
    ...
    Лист.Cells(1, 3).Value = "СрокДействия";
    ...
    Для i = 1 По РезультатЗапроса.Количество() Цикл
        ...
        Лист.Cells(i + 1, 3).Value = Выборка.СрокДействия;
    КонецЦикла;
...
Процедура СохранитьВDOCX(Путь)
    ...
    Документ.Content.Text = "Код;Наименование;СрокДействия" + Символы.ПС;
