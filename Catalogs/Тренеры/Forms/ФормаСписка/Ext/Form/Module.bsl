    ...
    Для Каждого Строка Из Строки Цикл
        ...
        Попытка
            ...
            Если Выборка.Следующий() Тогда
                СсылкаТренер = Выборка.Ссылка;
                ОбъектТренер = СсылкаТренер.ПолучитьОбъект();
                Сообщить("Обновлен существующий элемент: " + Код);
            Иначе
                ОбъектТренер = Справочники.Тренеры.СоздатьЭлемент();
                ОбъектТренер.Код = Код;
                Сообщить("Создан новый элемент: " + Код);
            КонецЕсли;
            
            // Заполнение реквизитов
            ОбъектТренер.Наименование = Наименование;
            ОбъектТренер.ДатаРождения = ДатаРождения;
            ОбъектТренер.НомерТелефона = НомерТелефона;
            ОбъектТренер.Email = Email;
            ОбъектТренер.Ставка = Ставка;
...
Процедура Выгрузить(Команда)
    ...
    ИмяФайлаПоУмолчанию = "Выгрузка_тренеров.txt";
...
Функция СформироватьТекстДляВыгрузкиНаСервере()
    // Получаем данные из справочника Тренеры
    Запрос = Новый Запрос;
    Запрос.Текст = 
        "ВЫБРАТЬ
        |    Тренеры.Код КАК Код,
        ...
        |    Тренеры.НомерТелефона КАК НомерТелефона,
        |    Тренеры.Email КАК Email,
        |    Тренеры.Ставка КАК Ставка
        |ИЗ
        |    Справочник.Тренеры КАК Тренеры";
