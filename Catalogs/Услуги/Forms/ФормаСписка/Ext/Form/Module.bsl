...
Процедура Загрузить(Команда)
    ...
    ДиалогОткрытияФайла.Фильтр = "Текстовые файлы (*.txt)|*.txt|Файлы Excel (*.xls, *.xlsx)|*.xls;*.xlsx|Файлы MXL (*.mxl)|*.mxl|CSV файлы (*.csv)|*.csv";
    ...
    Если ДиалогОткрытияФайла.Выбрать() Тогда
        ...
        ИначеЕсли Лев(Расширение, 4) = ".xls" Или Лев(Расширение, 5) = ".xlsx" Или Лев(Расширение, 4) = ".mxl" Тогда
...
Процедура ОбработатьДанныеНаСервере(Данные)
    ...
    Для Каждого Строка Из Строки Цикл
        ...
        Попытка
            ...
            Длительность     = Число(СокрЛП(Колонки[2]));
            Описание         = СокрЛП(Колонки[3]);
            ...
            Если Выборка.Следующий() Тогда
                СсылкаУслуги = Выборка.Ссылка;
                ОбъектУслуги = СсылкаУслуги.ПолучитьОбъект();
                Сообщить("Обновлен существующий элемент: " + Код);
            Иначе
                ОбъектУслуги = Справочники.Услуги.СоздатьЭлемент();
                ОбъектУслуги.Код = Код;
                Сообщить("Создан новый элемент: " + Код);
            КонецЕсли;
            
            // Заполнение реквизитов
            ОбъектУслуги.Наименование = Наименование;
            ОбъектУслуги.Длительность = Длительность;
            ОбъектУслуги.Описание     = Описание;
...
Процедура Выгрузить(Команда)
    ...
    ИмяФайлаПоУмолчанию = "Выгрузка_услуг.txt";
...
Функция СформироватьТекстДляВыгрузкиНаСервере()
    ...
    Заголовки = "Код;Наименование;Длительность";
    ...
    Пока Выборка.Следующий() Цикл
        Результат = Результат + 
            ...
            Строка(Выборка.Длительность) + Символы.ПС;
    КонецЦикла;
...
Процедура СохранитьВXLS(Путь)
    ...
    Лист.Cells(1, 3).Value = "Длительность";
    ...
    Для i = 1 По РезультатЗапроса.Количество() Цикл
        ...
        Лист.Cells(i + 1, 3).Value = Выборка.Длительность;
    КонецЦикла;
...
Процедура СохранитьВDOCX(Путь)
    ...
    Документ.Content.Text = "Код;Наименование;Длительность" + Символы.ПС;
