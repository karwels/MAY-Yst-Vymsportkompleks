...
Процедура ОбработатьДанныеНаСервере(Данные)
    ...
    Для Сч = 0 По Строки.Количество() - 1 Цикл
        ...
        // Проверяем, что строка содержит ровно 7 колонок (Номер, Дата, Менеджер, Оборудование, Количество, Цена, Сумма)
        Если Колонки.Количество() = 7 Тогда
            Попытка
                ...
                // Создаем новый документ "Закупка"
                НовыйДокумент = Документы.ЗакупкаОборудования.СоздатьДокумент();
                НовыйДокумент.Номер = Номер;
                НовыйДокумент.Дата = Дата;
                НовыйДокумент.Менеджер = Менеджер;
                
                // Добавляем строку в табличную часть "Закупка"
                НоваяСтрока = НовыйДокумент.Закупка.Добавить();
...
Процедура ЗакупкаЦенаПриИзменении(Элемент)
	СтрокаТабличнойЧасти = Элементы.Закупка.ТекущиеДанные;
...
Процедура Выгрузить(Команда)
    ...
    ИмяФайлаПоУмолчанию = Объект.Номер + "_ЗакупкаОборудования_Выгрузка.txt";
...
Функция СформироватьТекстДляВыгрузкиНаСервере()
    ...
    ЗаголовкиТабличнойЧасти = "Оборудование;Количество;Цена;Сумма";
    Результат = ЗаголовокДокумента + ЗаголовкиТабличнойЧасти + "
    |";
    
    // Данные табличной части
    ТабличнаяЧасть = Объект.Закупка;
...
Процедура СохранитьВXLS(Путь)
    ...
    Лист.Cells(2, 1).Value = "Менеджер: " + Объект.Менеджер;
    
    // Заголовки таблицы
    Лист.Cells(4, 1).Value = "Оборудование";
    Лист.Cells(4, 2).Value = "Количество";
    Лист.Cells(4, 3).Value = "Цена";
    Лист.Cells(4, 4).Value = "Сумма";
    
    // Данные таблицы
    Для i = 1 По Объект.Закупка.Количество() Цикл
        Строка = Объект.Закупка[i - 1];
        Лист.Cells(4 + i, 1).Value = Строка.Оборудование;
        Лист.Cells(4 + i, 2).Value = Строка.Количество;
        Лист.Cells(4 + i, 3).Value = Строка.Цена;
        Лист.Cells(4 + i, 4).Value = Строка.Сумма;
    КонецЦикла;
...
Процедура СохранитьВDOCX(Путь)
    ...
    // Добавляем таблицу
    Таблица = Документ.Tables.Add(Документ.Content, Объект.Закупка.Количество() + 1, 4);
